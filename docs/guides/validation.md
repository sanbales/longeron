...
| `evidence-drift` | warning | A `SourceEvidence` citation exists, but verification says the cited document drifted (sha256 mismatch) or the quote is lost (hash intact, text no longer contains it). Verification stays offline: URL documents check against the local evidence cache only, and an unreachable document stays silent -- the lint only speaks when it knows the citation broke. Models without citations are untouched. See the [evidence guide](evidence.md). |
| `unevidenced-value` | warning | Only under `evidence_coverage=True` / `--evidence-coverage`. A stated attribute value (a literal fact, per {func}`longeron.evidence.coverage`) carries no `SourceEvidence` citation. Off by default -- most models legitimately carry derived and assumed values, so coverage is a report and absence is not a defect (the provenance design's ratified posture). |
