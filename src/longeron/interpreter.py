...
Capabilities
============
* **Expression evaluation** over the AST in :mod:`longeron.ast` (arithmetic,
...
BUILTINS: dict[str, Any] = {
    ...
    "atan": math.atan,
    "pow": math.pow,
    "max": lambda *a: max(_flatten(a), default=None),
    "min": lambda *a: min(_flatten(a), default=None),
...
class Interpreter:
    """Evaluate and execute elements of a :class:`~longeron.model.Model`."""
...
class _ActionExecutor:
    #: observer hook (see longeron.replay): called as ``on_step(index,
...
class StateMachine:
    ...
    def __init__(
        ...
        self.now = 0.0
        self._firings = 0
        #: observer hook (see longeron.replay): called once after start()
