...
are loaded and the analysis surfaces are launched as main-area tabs::
    ...
    import longeron.widgets.app

    app = longeron.widgets.app.open()          # docks the sidebar panel
...
* the **loaded-models list**: one row per model (display name, source
  ...
  :func:`longeron.widgets.explorer.explore` tab; **Score** docks a requirements
...
docked item inspector (:mod:`longeron.widgets.inspector`) or a notebook cell --
...
(:meth:`ModelApp.refresh_explorers` -> :meth:`longeron.widgets.explorer.
...
:mod:`longeron.widgets.inspector`, built by :func:`open` and docked into the
...
* ``app.current_element`` -- the most recently selected
  ...
  public tree-selection hook (:meth:`longeron.widgets.explorer.TreeView.
  on_select`); scoreboard tabs report through the scoreboard widget's
  ``selected`` trait.  DIRECT :func:`longeron.widgets.explorer.explore` calls
...
try:
    ...
    raise MissingExtraError("longeron.widgets.app", "anywidget", "replay") from _err
...
#: DIRECT :func:`longeron.widgets.explorer.explore` calls are adopted into it so
...
class ModelApp(W.VBox):
    ...
    def explorers(self) -> tuple[Explorer, ...]:
        ...
        notebook cell).  Adopted: a DIRECT :func:`longeron.widgets.explorer.
    ...
    def refresh_explorers(self, model: M.Model) -> None:
        ...
        the selection's diagram (:meth:`longeron.widgets.explorer.Explorer.
    ...
    def _adopt_explorer(self, ex: Explorer) -> None:
        ...
        :func:`longeron.widgets.explorer.explore` calls made while this app is
    ...
    def _dock_scoreboard(self, entry: ModelEntry, widget: Any) -> Any:
        ...
        The explorer's :class:`~longeron.widgets.explorer._DockSweeper` rides
...
def open(
    ...
    * ``inspector`` -- also build the item inspector
      (:mod:`longeron.widgets.inspector`), docked into the RIGHT sidebar under
