...
seam (:mod:`longeron.widgets.app`, "THE INSPECTOR SEAM"): it subscribes to
...
sheet.  :func:`longeron.widgets.app.open` builds one by default and docks it
...
scoreboard tab updates the sheet live::
    ...
    app = longeron.widgets.app.open()      # left: the workbench; right: this sheet
...
* **relationship rows**: every relationship kind the explorer's tree
  ...
  :meth:`longeron.widgets.app.ModelApp.select_element` (an app-launched explorer
...
explorer tabs refresh their trees (:meth:`longeron.widgets.explorer.Explorer.
...
:class:`~longeron.widgets.app._AppSweeper`, ``side="right"``) closes a dead
...
:meth:`longeron.widgets.app.ModelApp._reveal_inspector_once`;
...
try:
    ...
    raise MissingExtraError("longeron.widgets.inspector", "anywidget", "replay") from _err
...
class Inspector(W.VBox):
    ...
    Built for one :class:`~longeron.widgets.app.ModelApp` (which exposes it as
    ...
    def reveal(self) -> None:
        ...
        selection (:meth:`longeron.widgets.app.ModelApp._reveal_inspector_once`;
