...
:func:`sysml2.analysis.viewer3d.mesh_viewer`) with plain ipywidgets into
...
* the 3D viewer with the top-N compromises in an adaptive grid
  (:func:`sysml2.analysis.geometry.lineup`: 4 -> 2x2, 6 -> 2x3), each
...
function render({ model, el }) {
  el.classList.add("sysml2-moefront");
  ...
  function draw() {
    ...
    make("line", { x1: M.left, y1: H - M.bottom, x2: W - M.right,
                   y2: H - M.bottom, class: "sysml2-moefront-axis" }, svg);
    make("line", { x1: M.left, y1: M.top, x2: M.left, y2: H - M.bottom,
                   class: "sysml2-moefront-axis" }, svg);
    ...
    text(M.left, H - M.bottom + 14, "sysml2-moefront-tick", P.xticks[0],
         "middle");
    text(W - M.right, H - M.bottom + 14, "sysml2-moefront-tick",
         P.xticks[1], "middle");
    text(M.left - 6, H - M.bottom + 3, "sysml2-moefront-tick", P.yticks[0],
         "end");
    text(M.left - 6, M.top + 4, "sysml2-moefront-tick", P.yticks[1], "end");
    text((M.left + W - M.right) / 2, H - 4, "sysml2-moefront-title",
         P.xlabel, "middle");
    const ylab = text(0, 0, "sysml2-moefront-title", P.ylabel, "middle");
    ...
    if (front.length > 1) {
      ...
      make("path", { d, class: "sysml2-moefront-step" }, svg);
    }
    for (const p of P.points) {
      const cls = p.front ? "sysml2-moefront-dot front"
        : p.feasible ? "sysml2-moefront-dot"
        : "sysml2-moefront-dot infeasible";
      if (p.pick) make("circle", { cx: sx(p.x), cy: sy(p.y), r: 7,
                                   class: "sysml2-moefront-pick" }, svg);
...
_SCATTER_CSS = """
.sysml2-moefront svg { display: block; width: 100%; height: auto; }
.sysml2-moefront-axis { stroke: #c4c7cc; stroke-width: 1; }
.sysml2-moefront-tick { fill: #6b7078; font-size: 9px;
  font-variant-numeric: tabular-nums; }
.sysml2-moefront-title { fill: #2b2d31; font-size: 10px; font-weight: 600; }
.sysml2-moefront-dot { fill: #c3c7cd; }
.sysml2-moefront-dot.infeasible { fill: #e3e5e8; }
.sysml2-moefront-dot.front { fill: #2f6b8f; stroke: #ffffff;
  stroke-width: 0.7; }
.sysml2-moefront-step { fill: none; stroke: #2f6b8f; stroke-width: 1.3;
  opacity: 0.85; }
.sysml2-moefront-pick { fill: none; stroke: #c2603e; stroke-width: 1.4; }
.sysml2-moefront { font-family: Helvetica, Arial, sans-serif; }
"""
