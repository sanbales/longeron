...
SYSML_STYLE: dict[str, dict[str, str]] = {
    ...
    " .sysml-definition > rect": {"fill": "#eef4fb", "stroke": "#4878a8",
                                  "rx": "4"},
    " .sysml-usage > rect": {"fill": "#f4faee", "stroke": "#6a9a48",
                             "rx": "4"},
    " .sysml-state > rect": {"fill": "#fdf6e3", "stroke": "#b58900",
                             "rx": "12"},
    " .sysml-step > rect": {"fill": "#f2eefb", "stroke": "#6c56a8",
                            "rx": "6"},
    " .sysml-marker > rect": {"fill": "#333333", "stroke": "#333333",
                              "rx": "8"},
    " .sysml-stereotype > text": {"fill": "#888888", "font-size": "9px"},
    " .sysml-attribute > text": {"font-size": "10px", "fill": "#444444"},
    " .sysml-edge-specializes > path": {"stroke": "#4878a8",
                                        "stroke-dasharray": "none"},
    " .sysml-edge-typed > path": {"stroke": "#6a9a48",
...
def _node(element: M.Element | None, title: str, css: str,
          stereotype: str | None = None) -> Node:
    labels = []
    if stereotype:
        labels.append(_label(f"\u00ab{stereotype}\u00bb", "sysml-stereotype"))
    labels.append(_label(title))
    node = Node(labels=labels, layoutOptions=dict(_NODE_LAYOUT),
...
def _finish(root: Node, style: dict | None = None,
            direction: str | None = None) -> Any:
...
def structure_diagram(element: M.Model | M.Namespace, *,
                      show_attributes: bool = True,
                      show_relationships: bool = True) -> Any:
...
class _StructureBuilder:
    ...
    def build(self) -> Node:
        ...
        roots = (self.element.members if isinstance(self.element, M.Model)
                 else [self.element])
        for member in roots:
    ...
    def _visit(self, element: M.Element) -> Node | None:
        if isinstance(element, M.Package):
            ...
            node = _node(element, element.label, "sysml-definition",
                         f"{stereotype} def".strip())
    ...
    def add_relationship_edges(self, root: Node) -> None:
        for element in list(self.nodes_elements()):
                ...
                for super_name in element.supers:
                    ...
                    if target is not None:
                        root.edges.append(_edge(
                            node, target, "sysml-edge-specializes",
                            end="generalization"))
            if isinstance(element, M.Usage):
                for type_name in element.types:
                    ...
                    if target is not None:
                        root.edges.append(_edge(
                            node, target, "sysml-edge-typed", end="arrow"))
        ...
        for element in list(self.nodes_elements()):
            for member in element.members if isinstance(element, M.Namespace) \
                    else []:
    ...
    def _connect_ends(self, root: Node, element: M.Usage) -> None:
        ends = (element.ends if hasattr(element, "ends") else
                [e for e in (getattr(element, "source_end", None),
                             getattr(element, "target_end", None)) if e])
        ...
        if len(resolved) >= 2 and all(n is not None for n in resolved):
            ...
            for source, target in itertools.pairwise(resolved):
                root.edges.append(_edge(source, target, "sysml-edge-connect",
                                        text=label))

    def _resolve_node(self, name: str, context: M.Element) -> Node | None:
        try:
            found = self.interp.resolver.resolve(
                name.split(".")[0], context.owner or self.model)
...
def _transition_text(transition: M.TransitionUsage) -> str | None:
    ...
    if transition.trigger is not None:
        ...
        if trigger.payload_types:
            bits.append(", ".join(t.split("::")[-1]
                                  for t in trigger.payload_types))
...
def _fill_states(container_node: Node, container: M.Definition | M.Usage,
    ...
    for member in container.members:
        if isinstance(member, M.Usage) and member.kind == "state" \
            ...
            if any(isinstance(m, M.Usage) and m.kind == "state"
...
def action_diagram(action: M.Definition | M.Usage) -> Any:
    ...
    if plan is not None:
        ...
        if plan.initial in steps:
            root.edges.append(_edge(steps["start"], steps[plan.initial],
                                    "sysml-edge-succession", end="arrow"))
        ...
        for member in action.members:
            if isinstance(member, (M.AssignmentAction, M.SendAction,
                                   M.AcceptAction, M.PerformAction,
                                   M.IfAction, M.WhileLoop, M.ForLoop,
                                   M.TerminateAction)) or (
                    isinstance(member, M.Usage) and member.kind == "action"):
                title = member.name or _statement_title(member)
                node = _node(member, title, "sysml-step",
                             type(member).__name__.replace("Action", ""))
                root.children.append(node)
                root.edges.append(_edge(previous, node,
                                        "sysml-edge-succession", end="arrow"))
                previous = node
        root.edges.append(_edge(previous, marker("done"),
                                "sysml-edge-succession", end="arrow"))
...
def on_select(diagram_widget: Any, model: M.Model,
              callback: Callable[[list[M.Element]], None]) -> None:
