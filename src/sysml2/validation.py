...
def validate(model: M.Model, *,
    ...
    return sorted(checker.diagnostics,
                  key=lambda d: (order[d.severity], d.element, d.code))
...
class _Checker:
    ...
    def check_all(self) -> None:
        for element in self.model.iter_tree():
            ...
            if isinstance(element, (M.Definition, M.Usage)) and \
                    element.kind == "state":
        ...
        for member in ns.members:
            for name in (member.name, member.short_name):
                ...
                if name in seen and seen[name] is not member:
                    self.report(
                        "error", "duplicate-name", member,
                        f"name {name!r} is already used by another member "
                        f"of {ns.label}")
                else:
                    seen[name] = member

    def _reference_fields(self, element: M.Definition | M.Usage
                          ) -> list[tuple[str, str]]:
    ...
    def _local_names(self, element: M.Namespace) -> set[str]:
        ...
        for item in element.iter_tree():
            ...
            elif isinstance(item, M.TransitionUsage) and item.trigger is not None \
                    and item.trigger.payload_name:
                names.add(item.trigger.payload_name)
        return names

    def _owned_expressions(self, element: M.Definition | M.Usage
                           ) -> list[tuple[M.Element, A.Expr]]:
        ...
        for item in element.members:
            ...
            elif isinstance(item, (M.Succession, M.TransitionUsage)) and \
                    item.guard is not None:
    ...
    def check_state_machine(self, machine: M.Definition | M.Usage) -> None:
        states = {m.name for m in machine.members
                  if isinstance(m, M.Usage) and m.kind == "state" and m.name}
        if not states:
            return
        transitions = [m for m in machine.members
                       if isinstance(m, M.TransitionUsage)]
        has_entry = any(t.source == M.ENTRY_SOURCE for t in transitions)
        if not has_entry:
            self.report("warning", "no-entry-transition", machine,
                        "state machine has states but no "
                        "'entry; then <state>;' transition")
        for transition in transitions:
            if transition.source not in (None, M.ENTRY_SOURCE) and \
                    transition.source not in states:
                self.report("error", "unknown-state", machine,
                            f"transition source {transition.source!r} is not "
                            f"a state of {machine.label}")
            if transition.target and transition.target not in states:
                self.report("error", "unknown-state", machine,
                            f"transition target {transition.target!r} is not "
                            f"a state of {machine.label}")
    ...
    def check_calc_result(self, calc: M.Definition | M.Usage) -> None:
        ...
        for member in calc.members:
            if isinstance(member, M.Usage) and member.direction == "return" \
                ...
                return
        if not calc.members and isinstance(calc, M.Usage) and \
                (calc.types or calc.subsets or calc.value):
            return  # reference/typed calc usages delegate their result
        self.report("warning", "calc-without-result", calc,
                    "calculation has no result expression")
...
def _expression_heads(expr: A.Expr) -> set[str]:
    ...
    def walk(node: A.Expr, bound: frozenset[str]) -> None:
        ...
        if isinstance(node, (A.Invocation, A.Constructor)):
            head = (node.target[0] if isinstance(node, A.Invocation)
                    else node.type[0])
        ...
        if isinstance(node, (A.CollectOp, A.SelectOp)):
            ...
            walk(node.body, bound)
            return
        for field_name in ("operand", "left", "right", "test", "then",
                           "orelse", "base", "index", "items"):
