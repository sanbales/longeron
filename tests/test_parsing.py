...
def test_parse_simple_package():
    result = longeron.parse_sysml_text("package P;")
    assert result.language == "sysml"


def test_parse_error_reports_position():
    with pytest.raises(longeron.ParseError) as exc:
...
def test_parse_error_trailing_garbage():
    with pytest.raises(longeron.ParseError):
        longeron.parse_sysml_text("package P; garbage garbage ((")


def test_unqualified_import_parses():
    # regression: upstream grammar required a visibility keyword before import
    longeron.parse_sysml_text("package P { import Other::*; }")


def test_entry_then_parses():
    # regression: upstream grammar needed 'then then S;' for entry transitions
    longeron.parse_sysml_text("state def S { entry; then off; state off; }")
...
def test_target_transition_then_clause_parses():
    ...
    longeron.parse_sysml_text(
        "state def S { state a; accept s : Sig do action D then b; state b; }"
    )
    longeron.parse_sysml_text("state def S { entry; then on; state on; then off; state off; }")


def test_library_package_without_standard_parses():
    # regression: upstream grammar required the full 'standard library package'
    # (grammar patch 7)
    longeron.parse_sysml_text("library package AHFCoreLib { part def X; }")
    longeron.parse_sysml_text("standard library package SL { part def X; }")
...
def test_metadata_prefixed_enum_value_parses():
    ...
    longeron.parse_sysml_text("package P { enum def L { a : L = 0; #Security enum b : L = 1; } }")
...
def test_one_line_multiline_note_parses():
    ...
    longeron.parse_sysml_text("package P { attribute h : Real = ( //* elided */ 4 ); }")
    longeron.parse_sysml_text("package P; // plain single-line note")
    longeron.parse_sysml_text("package P; //")
    longeron.parse_kerml_text("package K { //* elided */ class C; }")


def test_parse_expression():
    result = longeron.parse_expression_text("1 + 2 * x")
    assert result.tree is not None


def test_parse_expression_rejects_garbage():
    with pytest.raises(longeron.ParseError):
        longeron.parse_expression_text("1 + + ;;")


def test_parse_kerml():
    result = longeron.parse_kerml_text("""
        package Kernel {
            classifier Thing;
