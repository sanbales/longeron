...
def test_target_transition_then_clause_parses():
    ...
    sysml2.parse_sysml_text(
        "state def S { entry; then on; state on; then off; state off; }")
...
def test_named_send_action_parses():
    ...
    sysml2.parse_sysml_text(
        ...
        " action publish send new Publish(t, p) via publicationPort; } }")
...
def test_metadata_prefixed_enum_value_parses():
    ...
    sysml2.parse_sysml_text(
        "package P { enum def L { a : L = 0; #Security enum b : L = 1; } }")
...
def test_one_line_multiline_note_parses():
    ...
    sysml2.parse_sysml_text(
        "package P { attribute h : Real = ( //* elided */ 4 ); }")
