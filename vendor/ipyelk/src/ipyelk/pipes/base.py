...
class _CompletedTask:
    """Duck-typed stand-in for an :class:`asyncio.Task` that never ran.

    LOCAL PATCH (sysml2-experiments): returned by ``Pipe.schedule_run`` when
    no event loop is running (plain scripts, pytest).  The pipeline's layout
    stage is a browser round-trip (elkjs runs in the frontend), so there is
    nothing useful to execute headlessly; done-callbacks are swallowed so
    downstream view updates are not fed empty layouts.
    """

    def add_done_callback(self, callback: Callable) -> None:
        pass  # nothing ran; nothing to report
...
class Pipe(W.Widget):
    ...
    def schedule_run(self, change: T.Bunch = None) -> asyncio.Task:
        ...
        try:
            ...
            # LOCAL PATCH (sysml2-experiments): outside a running event loop
            # (plain scripts, pytest, nbclient teardown) there is no frontend
            # to lay the diagram out, so skip scheduling instead of raising.
            # The coroutine is deliberately never created here, avoiding
    ...
    def _post_run(self, future: asyncio.Future):
        ...
        if exception is None:
            return
        if not isinstance(self.status, PipeStatus) or self.status.exception is not exception:
            self.status = PipeStatus.error(start_time=datetime.now(), exception=exception)
